...
new_submissions_counter = Counter('new_submissions', 'Counter of new submissions')
num_comments_counter = Counter('num_comments', 'Counter of number of comments')
hour_histogram = Histogram('hour', 'Hour of new submissions', buckets=range(0, 24, 1))
post_length_histogram = Histogram('post_len', 'Post length of new submissions', buckets=range(0, 1000, 100))
submission_fetch_time = Histogram('submission_fetch_time', 'Submission fetch times in seconds')
...
def submission_fetching(self, subreddit_name='all', num_posts=10, only_new=True, filename=None):
    ...
    new_submissions_counter.inc(num_fetched)
    num_comments_counter.inc(sum_comments)
    for index, post in posts.iterrows():
        post_length_histogram.observe(round(len(post.post_text), -2))
        hour_histogram.observe(int(post['date_posted'].hour))
